...
//        $sql = "select * from consreporte where fecha >='".$fecha1."' and fecha<='".$fecha2."'".$cadusuario1;
        $sql = "
                    ...
                    (select
                    date(p.prueba_fechacuenta) as fecha,
                    time(p.prueba_fechacuenta) as hora,
                    concat('Prueba: A cuenta ',t.tipoprueba_descripcion,', ', e.paciente_nombre) as detalle,
                    p.prueba_acuenta as ingresos,
                    0 as egresos,
                    0 as utilidad,
                    ...
                    from prueba p
                    left join tipo_prueba t on t.tipoprueba_id = p.tipoprueba_id
                    left join paciente e on e.paciente_id = p.paciente_id

                    where 
                    date(p.prueba_fechacuenta) >= '".$fecha1."' and
                    date(p.prueba_fechacuenta) <= '".$fecha2."' )

                    UNION

                    (
                    select
                    date(p.prueba_fechasaldo) as fecha,
                    time(p.prueba_fechasaldo) as time,
                    concat('Prueba: Saldo ',t.tipoprueba_descripcion,', ', e.paciente_nombre) as detalle,
                    p.prueba_saldo as ingresos,
                    0 as egresos,
                    0 as utilidad,
                    ...
                    where 
                    date(p.prueba_fechasaldo) >= '".$fecha1."' and
                    date(p.prueba_fechasaldo) <= '".$fecha2."'
                    )
                    ...
                    concat('Ingreso: Saldo ',i.ingreso_concepto,', ', i.ingreso_nombre) as detalle,
                    ...
                    where
                    date(i.ingreso_fecha) >= '".$fecha1."' and
                    date(i.ingreso_fecha) <= '".$fecha2."'
                    ...
                    concat('egreso: Saldo ',e.egreso_concepto,', ', e.egreso_nombre) as detalle,
                    0 as ingresos,
                    ...
                    where
                    date(e.egreso_fecha) >= '".$fecha1."' and
                    date(e.egreso_fecha) <= '".$fecha2."'
                    )";
